...
To open my coursework project read this Readme and clone this Repo and Step1: cd vaultkey Step2: python3 ./run.py
...
- `POST /api/auth/certificate/challenge` — request a nonce for certificate challenge-response login
- `POST /api/auth/certificate/verify` — verify a signed nonce with the client-side private key
...
- The server issues a fresh nonce for each certificate login attempt.
- The client signs the nonce locally with the private key and returns the signature.
- The nonce prevents replay attacks and the private key stays client-side.
- Revoked certificates are blocked during login, document signing, and document verification.

To run the certificate login helper:

```bash
python tools/cert_login_client.py
```

Video demo link can stay as [To be added].
