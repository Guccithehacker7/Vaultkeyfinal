...
class User(db.Model):
    ...
    certificate_login_challenges = db.relationship("CertificateLoginChallenge", back_populates="user", cascade="all, delete-orphan", lazy="select")
...
class CertificateLoginChallenge(db.Model):
    __tablename__ = "certificate_login_challenge"

    id         = db.Column(GUID(), primary_key=True, default=uuid.uuid4)
    user_id    = db.Column(GUID(), db.ForeignKey("user.id"), nullable=False)
    nonce      = db.Column(db.String(128), nullable=False, unique=True, index=True)
    created_at = db.Column(db.DateTime(timezone=True), nullable=False, default=utcnow)
    expires_at = db.Column(db.DateTime(timezone=True), nullable=False)
    used       = db.Column(db.Boolean, nullable=False, default=False)

    user = db.relationship("User", back_populates="certificate_login_challenges")

    def __repr__(self):
        return f"<CertificateLoginChallenge user={self.user_id} used={self.used}>"
