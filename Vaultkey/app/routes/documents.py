...
from app.models import Certificate, CertificateStatus, DocumentSignature, StoredDocument
...
def sign_document():
    ...
    if not active_certificate:
        return jsonify({"message": "No active certificate available for signing"}), 403
...
def verify_document():
    ...
    if certificate_record.status != CertificateStatus.ACTIVE:
