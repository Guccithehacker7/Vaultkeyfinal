...
@auth_bp.route("/api/auth/certificate/challenge", methods=["POST"])
def certificate_challenge():
    ...
    if not identifier:
        return jsonify({"message": "username_or_email is required"}), 400
    ...
    return jsonify({"nonce": nonce, "expires_at": expires_at.isoformat()}), 200


@auth_bp.route("/api/auth/certificate/verify", methods=["POST"])
def certificate_verify():
    ...
    if not identifier or not certificate_pem or not nonce or not signature_b64:
        return jsonify({"message": "username_or_email, certificate_pem, nonce, and signature are required"}), 400
    ...
    challenge = (
        CertificateLoginChallenge.query
        .filter_by(user_id=user.id, nonce=nonce)
        .order_by(CertificateLoginChallenge.created_at.desc())
        .first()
    )
    if not challenge:
        return jsonify({"message": "Invalid challenge nonce"}), 401
    if challenge.used:
        return jsonify({"message": "Challenge nonce has already been used"}), 401
    if _ensure_aware(challenge.expires_at) <= datetime.now(timezone.utc):
        return jsonify({"message": "Challenge nonce has expired"}), 401
    ...
    try:
        ...
        log_audit(action="LOGIN_FAILURE", status="FAILURE", user_id=user.id, ip_address=_get_client_ip())
        return jsonify({"message": str(exc)}), 401

    cert_record = Certificate.query.filter_by(serial_number=str(certificate.serial_number)).first()
    if not cert_record:
        log_audit(action="LOGIN_FAILURE", status="FAILURE", user_id=user.id, ip_address=_get_client_ip())
        return jsonify({"message": "Certificate is not registered"}), 401
    if cert_record.status != CertificateStatus.ACTIVE:
        log_audit(action="LOGIN_FAILURE", status="FAILURE", user_id=user.id, ip_address=_get_client_ip())
        return jsonify({"message": "Certificate has been revoked and cannot be trusted"}), 401
    if cert_record.user_id != user.id:
        log_audit(action="LOGIN_FAILURE", status="FAILURE", user_id=user.id, ip_address=_get_client_ip())
        return jsonify({"message": "Certificate does not belong to this user"}), 401

    if not verify_signature(certificate.public_key(), signature, nonce.encode("utf-8")):
        log_audit(action="LOGIN_FAILURE", status="FAILURE", user_id=user.id, ip_address=_get_client_ip())
